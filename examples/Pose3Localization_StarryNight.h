...
public:
  ...
  FileUtils(std::string config_filename, std::string output_filename) {
    ...
    output_file_poses_ = "../results/starry_night_results/" + output_filename + "_poses";
    output_file_poses_dr_ = "../results/starry_night_results/" + output_filename + "_poses_dr";
    output_file_landmarks_ = "../results/starry_night_results/" + output_filename + "_landmarks";
    output_file_marginals_ = "../results/starry_night_results/" + output_filename + "_marginals";
    output_file_groundtruth_ = "../results/starry_night_results/" + output_filename + "_groundtruth";
  }
...
static FileUtils::ConfigParams getParamsFromConfig(const YAML::Node& config) {
  ...
  params.interpDuringSolve = config["interp_during_solve"]["enable"].as<bool>();
  params.interpSkipStates = config["interp_during_solve"]["interp_skip_states"].as<size_t>();

  params.interpAfterSolve = config["interp_after_solve"]["enable"].as<bool>();
  params.interpCovariance = config["interp_after_solve"]["interp_covariance"].as<bool>();

  // params validity checks
  if (params.interpDuringSolve && !params.useWNOAFactor) {
    std::cerr << "Error: interp_during_solve requires WNOA factor to be enabled." << std::endl;
    exit(1);
  }
  if (params.interpAfterSolve && !params.useWNOAFactor) {
    std::cerr << "Error: interp_after_solve requires WNOA factor to be enabled." << std::endl;
    exit(1);
  }
  if (params.interpAfterSolve && !params.interpDuringSolve) {
    std::cerr << "Error: interp_after_solve requires interp_during_solve to be enabled." << std::endl;
    exit(1);
  }
...
static auto loadAllData() {
  ...
  return std::make_tuple(inputs, gtPoses, gtLandmarks, Cal, T_cv, odomNoise, stereoNoise, measTripleVector);
}
...
void saveAllResultsToFile(size_t numPoses,
                       size_t numLandmarks,
