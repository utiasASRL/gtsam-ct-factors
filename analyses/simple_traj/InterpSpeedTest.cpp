...
struct InterpExampleParams {
  Vector3 cov_diag_unary;  // unary prior covariance
  Vector3 Q_wnoa;          // WNOA Power Spectral Density
  Vector3 vel_mean;        // mean velocity
  int N;                   // total number of states
  int period_interp;  // interpolation period (num interp points between est
                      // points)
  double del_t;       // timestep
  bool perturb_meas;  // select to peturb measurements
  bool fixed_noise;   // select to fix noise
  int n_runs;         // number of runs to perform
};
...
int main(int argc, char* argv[]) {
  ...
  cout << "Small Angle Approx Off:" << endl;
  runInterpExample(p, run_full_opt, false);
  cout << "Small Angle Approx On:" << endl;
  runInterpExample(p, run_full_opt, true);
}